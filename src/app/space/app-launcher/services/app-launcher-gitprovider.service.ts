...
export class AppLauncherGitproviderService implements GitProviderService {
    ...
    private get options(): Observable<RequestOptions> {
      ...
      return Observable.fromPromise(this.tokenProvider.token.then((token) => {
        // TODO : pass token if needed to test
        // token = 'token';
  ...
  connectGitHubAccount(redirectUrl: string): void {
    ...
    //this.providerService.linkGitHub(redirectUrl);
   ...
   * @returns {Headers}
   */
  // getGitHubHeaders(): Observable<Headers> {
  //   return Observable.fromPromise(this.tokenProvider.gitHubToken.then((token) => {
  //     let newHeaders =  cloneDeep(AppLauncherGitproviderService.HEADERS);
  //     newHeaders.set('X-Git-Token', `${token}`);
  //     newHeaders.set('Authorization', 'Bearer token');
  //     newHeaders.set('x-app', this.ORIGIN);
  //     newHeaders.set('x-git-provider', this.PROVIDER);
  //     return newHeaders;
  //   }));
  // }
